...
Task 7 (R-0013): new Render pserv `fortel2-replica-reth` + disk + staging gateway. Do **not** re-apply the live `fortel2-replica` Blueprint entry or point it at `Dockerfile.reth`. ForteL2 `tasks/` / `rail-interface.json` / `replica/FRIENDS.md` are planner-owned — do not edit them from this repo.
