...
Docker-only OP Stack **verifier node** for the ForteL2 learning L2 (chain ID **852**), deriving from Ethereum **Sepolia**. The runtime is two OP Labs binaries (`op-reth` + `op-node`) via `docker-compose.yml` locally, or the root `Dockerfile` on Render (pinned by digest; entrypoint asserts `Reth Version: 2.3.0-dev` commit `9384bc53…`). Supporting Python in-repo: `rpc-method-filter.py`, `l1_rpc_router.py`, `entrypoint.sh`. Public doors: `gateway/` (nginx replica proxy) and `sequencer-read/start.sh` (filtered sequencer tip). Unit tests: `python3 -m unittest discover -s tests -v` (or `make test`). See `README.md` for the full quick start and smoke-test commands.

Task 7 (R-0013): new Render pserv `fortel2-replica-reth` + disk + staging gateway. Do **not** re-apply the live `fortel2-replica` Blueprint entry. Disable auto-deploy on that live pserv before this image reaches main. ForteL2 `tasks/` / `rail-interface.json` / `replica/FRIENDS.md` are planner-owned — do not edit them from this repo.
...
- **Render memory (R-0012 live geth / R-0013 new reth):** Starter (512MB) OOMs. Live geth stays Wave 1 on Standard until Phase C. New reth pserv starts Standard with `RETH_CROSS_BLOCK_CACHE_MB=256` (pinned op-reth default is 4096 MB — OOM), `RETH_RPC_CACHE_MAX_BLOCKS=256`, `OP_NODE_GOMEMLIMIT=768MiB`, `L1_CACHE_SIZE=128`, `L1_MAX_CONCURRENCY=2`, `L1_RPC_MAX_BATCH_SIZE=5`. `GETH_*` knobs do nothing on op-reth (Rust). Measure Phase B before any plan change. Never `L1_CACHE_SIZE=0`. Initial sync `L1_RPC_FORCE=metered` (D-0105). Daily Cloud Agent **Daily replica health** (04:00 Pacific, skill `.cursor/skills/daily-replica-health`) still scores the live geth replica; it does not change env. It uses Cloud Render plus QuickNode — do not run a local overnight loop.
...
- The single-container `Dockerfile`/`entrypoint.sh` (Render deploy) publishes the **method filter** on `PORT`/`L2_HTTP_PORT` (often `10000` on the Private Service); op-reth HTTP is loopback `:8546` and op-node RPC is loopback `:9545` (never public). Local `docker-compose.yml` still publishes host ports `9545`/`9547` to raw op-reth/op-node. Don't confuse the two port schemes.
- Live Render replica is a **Private Service** (`http://fortel2-replica:10000`, 50 GB disk, geth until Phase C). Do not convert it to Web or re-apply the live `render.yaml` entry (would create a second pserv with an empty disk). Task 7 adds `fortel2-replica-reth` + `fortel2-replica-reth-rpc` (R-0013); live `fortel2-replica-rpc` stays Dashboard-created (R-0008). Public replica: `https://fortel2-replica-rpc.onrender.com` (`./gateway/Dockerfile`). Public sequencer tip: `https://fortel2-sequencer-rpc.onrender.com` (`sequencer-read/start.sh`, R-0009). Do not point a Web Service at the root `./Dockerfile`.
