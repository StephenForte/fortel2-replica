...
- **Render env vars:** Blueprints usually only seed **new** services. On an existing Private Service, copy every var from `README.md` (Render → Manual setup) into the dashboard — especially `GETH_CACHE_MB`, `GETH_GOMEMLIMIT`, `OP_NODE_GOMEMLIMIT`, and the `L1_RPC_*` schedule keys — then Manual Deploy.
- **Render memory:** Starter (512MB) OOMs. Standard needs the memory env vars above; L1 derivation bursts (`decoded singular batch from channel`) are the usual trigger. Upgrade to Pro if exit 137 persists after tuning.
