...
- **L1 credit knobs:** `L1_HTTP_POLL_INTERVAL` (default `24s`) and `L1_RPC_RATE_LIMIT` (default `5`) are passed to op-node. Near QuickNode credit caps, set `L1_USE_PUBLIC_RPC=1` so entrypoint overrides `L1_RPC_URL` with publicnode (default `https://ethereum-sepolia-rpc.publicnode.com`). Use a Render-only QuickNode URL in `L1_RPC_URL` — not the Mac mini sequencer endpoint.
