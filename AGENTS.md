...
- **L1 credit knobs:** `L1_HTTP_POLL_INTERVAL` (default `12s`) and `L1_RPC_RATE_LIMIT` (default `20`) are passed to op-node. Use a Render-only QuickNode URL — not the Mac mini sequencer endpoint.
