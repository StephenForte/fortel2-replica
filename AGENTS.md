...
- **Render memory:** Starter (512MB) OOMs. Standard needs low geth cache + Go soft caps — Blueprint sets `GETH_CACHE_MB=128`, `GETH_GOMEMLIMIT=700MiB`, `OP_NODE_GOMEMLIMIT=768MiB`. L1 derivation bursts (`decoded singular batch from channel`) are the usual trigger; upgrade to Pro if exit 137 persists.
