...
- **Render memory:** Starter (512MB) OOMs. Standard needs `GETH_CACHE_MB=128`, `GETH_FDLIMIT=4096`, `GETH_GOMEMLIMIT=700MiB`, `OP_NODE_GOMEMLIMIT=768MiB`, `L1_CACHE_SIZE=128`, `L1_MAX_CONCURRENCY=2`, `L1_RPC_MAX_BATCH_SIZE=5`. L1 derivation bursts (`decoded singular batch from channel`) plus op-node’s default 900-block L1 receipt cache are the usual trigger. Upgrade to Pro if exit 137 persists after tuning.
