...
Task 7 (R-0017): Phase C executed. Live public read (`https://fortel2-replica-rpc.onrender.com`) and SOS private read serve `fortel2-replica-reth` (`Dockerfile.reth`). The geth EL surface is retired in this repo (Task 9). The geth pserv `fortel2-replica` (with its 50 GB disk) and the staging gateway `fortel2-replica-reth-rpc` were **deleted from Render 2026-09-14** (R-0019). There is no geth rollback resource: recovering that path means a new disk and a full resync. Render private hostnames are immutable slugs — never rename services (R-0013 rename-swap does not move traffic). Do **not** re-apply this Blueprint (D-0128: a new apply creates empty-disk services). ForteL2 `tasks/` / `rail-interface.json` / `replica/FRIENDS.md` are planner-owned — do not edit them from this repo.
...
- The single-container `Dockerfile.reth` / `entrypoint-reth.sh` publishes the **method filter** on `PORT`/`L2_HTTP_PORT` (often `10000` on the Private Service); op-reth HTTP is loopback `:8546` and op-node RPC is loopback `:9545` (never public). Local `docker-compose.yml` still publishes host ports `9545`/`9547` to raw op-reth/op-node (`docker compose --profile render build replica-reth` builds the Render image). Don't confuse the two port schemes.
- Live public read is reth via `fortel2-replica-rpc` → `http://fortel2-replica-reth:10000` (R-0017). Hostnames are immutable slugs — never rename services. Do not convert the pserv to Web or re-apply `render.yaml` (would create a second pserv with an empty disk). Task 7 added `fortel2-replica-reth` + `fortel2-replica-reth-rpc` (R-0013); live `fortel2-replica-rpc` stays Dashboard-created (R-0008). Public replica: `https://fortel2-replica-rpc.onrender.com` (`./gateway/Dockerfile`). Public sequencer tip: `https://fortel2-sequencer-rpc.onrender.com` (`sequencer-read/start.sh`, R-0009). Do not point a Web Service at `Dockerfile.reth`.
