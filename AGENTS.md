...
Docker-only OP Stack **verifier node** for the ForteL2 learning L2 (chain ID **852**), deriving from Ethereum **Sepolia**. The runtime is two OP Labs binaries (`op-geth` + `op-node`) via `docker-compose.yml` locally, or the root `Dockerfile` on Render. Supporting Python in-repo: `rpc-method-filter.py`, `l1_rpc_router.py`, `entrypoint.sh`. Unit tests: `python3 -m unittest discover -s tests -v` (or `make test`). See `README.md` for the full quick start and smoke-test commands.
...
- **Render memory:** Starter (512MB) OOMs. Standard needs `GETH_CACHE_MB=128`, `GETH_GOMEMLIMIT=700MiB`, `OP_NODE_GOMEMLIMIT=768MiB`. L1 derivation bursts (`decoded singular batch from channel`) are the usual trigger. Upgrade to Pro if exit 137 persists after tuning.
- **L1 credit knobs:** `L1_HTTP_POLL_INTERVAL` (default `24s`) and `L1_RPC_RATE_LIMIT` (default `5`) are passed to op-node. `L1_RPC_SCHEDULE=business` starts `/l1_rpc_router.py` (needs `python3` + `tzdata`) and points op-node at `http://127.0.0.1:18545`, which forwards to QuickNode **09:00–17:00** in `TZ` (default `America/Los_Angeles`) and publicnode otherwise. Overrides: `L1_RPC_FORCE=public|metered` or `L1_USE_PUBLIC_RPC=1`. Use a Render-only QuickNode URL in `L1_RPC_URL` — not the Mac mini sequencer endpoint.
...
- Live Render replica is a **Private Service** (`http://fortel2-replica:10000`, 50 GB disk). Do not convert it to Web or apply `render.yaml` as a new Blueprint onto that live service (would create a second pserv with an empty 20 GB disk). Greenfield Blueprint apply uses `type: pserv` in `render.yaml`. No public replica URL yet (ForteL2 D-0031). A public door is a **separate diskless** Web Service (`fortel2-replica-rpc`, `./gateway/Dockerfile` once that path exists) — see README **Going public**. Do not point a Web Service at the root `./Dockerfile`.
