...
**The parity check's whole design is one sentence: the reference is a comparator, not the chain.** A friend whose node disagrees with the operator's endpoint must not conclude their node is wrong, because the interesting case for a learning chain is the one where the operator's node is wrong. So a mismatch prints `DIVERGENCE: you and this reference disagree` and exits non-zero without issuing a verdict; the script is read-only, never writes the reference into a datadir, never feeds it into derivation, and never mentions `debug_setHead`. Ambiguity fails closed with a **named** cause rather than passing: `REFERENCE_UNREACHABLE`, `REFERENCE_NULL`, `REFERENCE_CHAIN_ID`. Verified in `scripts/check_friend_parity.py` — every ambiguous branch routes through `_fail(3, …)`, so no unreachable or null reference can exit 0.
